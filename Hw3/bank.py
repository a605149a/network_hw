...
def open_database(path='bank.db'):
    ...
    if new:
        ...
        add_payment(db, 'liu', 'yu', 5000, 'Registration for PyCon')
        add_payment(db, 'liu', 'yu', 5000, 'Payment for writing that code')
        add_payment(db, 'yu', 'liu', 100, 'Gas money-thanks for the ride!')
        db.commit()
    return db
